...
func GetPatientById(c *gin.Context) {
	...
	db.Find(&patientViewModel.PatientAppointments, patientId)
	db.Find(&patientViewModel.PatientTreatments, patientId)
	db.Find(&patientViewModel.PatientTreatmentDetails, patientId)
	db.Find(&patientViewModel.PatientReports, patientId)
...
func CreatePatient(c *gin.Context) {
	...
	UpdatePatientIDInArrays(patientViewModel.PatientAppointments, patientId.Int64)
	db.Create(&patientViewModel.PatientAppointments)
	UpdatePatientIDInArrays(patientViewModel.PatientReports, patientId.Int64)
	db.Create(&patientViewModel.PatientReports)
	UpdatePatientIDInArrays(patientViewModel.PatientTreatments, patientId.Int64)
	db.Create(&patientViewModel.PatientTreatments)
	UpdatePatientIDInArrays(patientViewModel.PatientTreatmentDetails, patientId.Int64)
...
func UpdatePatientIDInArrays[pa PatientArray](patientArray []*pa, patientId int64) {
	if !(patientArray == nil || len(patientArray) < 1) {
		...
		for _, arrayVal := range patientArray {
			...
			if reflect.TypeOf(arrayVal).Kind().String() == "PatientTreatment" {
				for _, ptd := range pvm.PatientTreatmentDetails {
					if ptd.PatientTreatmentID.Int64 == value.Int() {
						ptd.PatientTreatmentID.Int64 = reflect.ValueOf(&arrayVal).Elem().FieldByName("ID").Int()
					}
				}
			}
		}
